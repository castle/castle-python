...
def default_command_with_data(**data):
    """What we expect the log command to look like."""
    return Command(method='post', path='log', data=dict(sent_at=mock.sentinel.timestamp, **data))


class CommandsLogTestCase(unittest.TestCase):
    def setUp(self):
        # patch timestamp to return a known value
        timestamp_patcher = mock.patch('castle.commands.log.generate_timestamp.call')
    ...
    def test_call(self):
        ...
        self.assertEqual(CommandsLog(context).call(options), expected)
