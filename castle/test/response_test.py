...
class ResponseTestCase(unittest.TestCase):
    def response(self, status_code=200, body=None):
    ...
    def test_response_none(self):
        self.assertEqual(Response(self.response()).call(), {})

    def test_response_empty(self):
        self.assertEqual(Response(self.response(body=b'')).call(), {})

    def test_response_authenticate(self):
        self.assertEqual(Response(self.response(body=b'{"action":"allow","user_id":"12345"}')).call(), {"action":"allow","user_id":"12345"})

    def test_verify_200_299(self):
        for status_code in range(200, 299):
            self.assertEqual(Response(self.response(status_code=status_code)).verify(), None)

    def test_verify_400(self):
        with self.assertRaises(BadRequestError):
            Response(self.response(status_code=400)).verify()

    def test_verify_401(self):
        with self.assertRaises(UnauthorizedError):
            Response(self.response(status_code=401)).verify()

    def test_verify_403(self):
        with self.assertRaises(ForbiddenError):
            Response(self.response(status_code=403)).verify()

    def test_verify_404(self):
        with self.assertRaises(NotFoundError):
            Response(self.response(status_code=404)).verify()

    def test_verify_419(self):
        with self.assertRaises(UserUnauthorizedError):
            Response(self.response(status_code=419)).verify()
